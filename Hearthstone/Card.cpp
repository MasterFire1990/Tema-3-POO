...
void Card::setName(const std::string& n) {
    if (n.empty()) throw HearthstoneException("Card name cannot be empty");
    name = n;
}

void Card::setManaCost(int cost) {
    if (cost < 1 || cost > 10)
        throw HearthstoneException("Mana cost must be between 1 and 10");
    manaCost = cost;
}
...
Card& Card::operator=(const Card& other) {
    if (this != &other) { name = other.name; manaCost = other.manaCost; }
    return *this;
}
